 ...
 */
export async function joinGasDaddyPlan(authorization, address) {
  try {
    console.log('🚀 Setting up EIP-7702 delegation...');
    console.log('👤 User EOA:', address);
    ...
    if (
      ...
      throw new Error(
        'Invalid authorization format. Required: contractAddress, chainId, nonce, r, s, yParity'
      );
    }
    ...
    const hash = await sponsorWalletClient.sendTransaction({
      ...
      to: address, // send to user's EOA address
